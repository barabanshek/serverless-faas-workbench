...
class Greeter(fibonacci_pb2_grpc.GreeterServicer):
    ...
    def __init__(self):
        ...
        self.tfidf_vect = TfidfVectorizer(min_df=100).fit(df['train'])
    ...
    def predict(self, input_data):
        ...
        X = self.tfidf_vect.transform(df_input['x'])
    ...
    def SayHello(self, request, context):
        pred, lat = self.predict(request.name)
        msg = "fn: Model Serving LR | input: %s, pred: %i, lat: %i | runtime: python" % (request.name, pred, lat)
