...
# Convert the relevant columns to appropriate data types
data['REAP_Restore'] = pd.to_numeric(data['REAP_Restore'], errors='coerce')
data['Accelerated_Restore'] = pd.to_numeric(data['Accelerated_Restore'], errors='coerce')
...
for i, app in enumerate(data['Application']):
    # Vanila times
    # vanila_total = data.loc[i, 'REAP_Restore']
    # ax.bar(i - width/1.8, data.loc[i, 'REAP_Restore'], width=width, color=vanila_colors[0], label='REAP' if i == 0 else "", zorder=3)
    ...
    # Accelerated times
    vanila_total = 0
    accelerated_total = data.loc[i, 'Accelerated_Restore']
    ax.bar(i + width/1.8, data.loc[i, 'Restoration_Speedup_over_REAP_percent'], width=width, color=accelerated_colors[1], label='Sabre' if i == 0 else "", zorder=3, hatch='')
    ...
    # compression_ratio = data.loc[i, 'Restoration_Speedup_over_REAP_percent']
    # # Positioning the annotation above the highest bar
    # highest_point = max(vanila_total, accelerated_total)
    # ax.text(i, highest_point, f'{compression_ratio:.2f}%', ha='center', va='bottom', zorder=4, fontsize=text_size_big, color='black')
...
ax.set_ylabel('Memory Restoration Speedup \n over REAP (%)', fontsize=text_size_ultrabig)
...
plt.savefig("mem_restore_results_only.png")
plt.savefig("mem_restore_results_only.pdf", format="pdf", bbox_inches="tight")
