...
file_path = 'Downloads/raw_data-REAP.csv'  # Replace with your file path
...
data.columns = [
    ...
    'VanilaVMLoad_us', 'VanilaRestore', 'VanilaColdStart_us', 'VanilaTotal_us',
    'AcceleratedVMLoad_us', 'AcceleratedRestore', 'AcceleratedColdStart_us', 'AcceleratedTotal_us',
    'SnapshotSize', 'CompressionRatio', 'SpeedupPercentage', 'SpeedupRestoration'
]
...
for i, app in enumerate(data['Application']):
    ...
    ax.bar(i - width/1.8, data.loc[i, 'VanilaVMLoad_us'], width=width, color=vanila_colors[0], label='Baseline VM load with REAP prefetching' if i == 0 else "", zorder=3)
    ...
    ax.bar(i + width/1.8, data.loc[i, 'AcceleratedVMLoad_us'], width=width, color=accelerated_colors[0], label='Our VM load with prefetching' if i == 0 else "", zorder=3, hatch='o')
...
ax.set_ylabel('End-to-end cold start time, ms', fontsize=text_size_ultrabig)
