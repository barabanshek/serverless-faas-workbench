...
for i, app in enumerate(data['Application']):
    ...
    ax.bar(i + width/1.8, data.loc[i, 'Accelerated_Snapshot_Size_MB'], width=width, color=accelerated_colors[1], label='Sabre' if i == 0 else "", zorder=3, hatch='')
...
ax.set_ylabel('Snapshot Size (MB)', fontsize=text_size_ultrabig)
...
plt.savefig("snapshot_size_results.png")
plt.savefig("snapshot_size_results.pdf", format="pdf", bbox_inches="tight")
