...
for i, app in enumerate(data['Application']):
    ...
    ax.bar(i + width/1.8, data.loc[i, 'Accelerated_Restore'], width=width, color=accelerated_colors[1], label='Sabre' if i == 0 else "", zorder=3, hatch='')
...
ax.set_ylabel('Memory Restoration Latency (ms)', fontsize=text_size_ultrabig)
...
plt.savefig("mem_restore_results.png")
plt.savefig("mem_restore_results.pdf", format="pdf", bbox_inches="tight")
